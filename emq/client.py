...
def client_main(dev_id, key):
    """
    客户端发布消息
    :param message: 消息主体
    :return:
    """
    # devices = Device.objects.get(pk=dev_id)

    time_now = time.strftime('%Y-%m-%d %H-%M-%S', time.localtime(time.time()))

    # for device in devices:
    # if Device.objects.filter(id=dev_id, authentication=key)
    data = random.uniform(20, 30)
    data = round(data, 1)
    payload = {"device_id": dev_id, 'data': data, "time": time_now, "key": key}
